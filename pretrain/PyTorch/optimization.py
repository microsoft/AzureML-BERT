...
def warmup_linear_decay_exp(
    global_step, decay_rate, decay_steps, total_steps, warmup=0.002
):
...
class LinearWarmupExponentialSchedule:
    def __init__(
        ...
        self.decay_steps = int(
            math.ceil(
                (math.log(self.decay_rate) / math.log(final_lr / initial_lr))
                * (1.0 - warmup)
                * t_total
            )
        )
    ...
    def get_lr(self, global_step):
        ...
        return self.decay_rate ** (
            (global_step - self.warmup_end) / self.decay_steps
        )
